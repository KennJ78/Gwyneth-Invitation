...
(function () {
  ...
  // Countdown to October 31, 2:00 PM (local time). If date has passed this year, target next year.
  (function initCountdown() {
    ...
    let target = new Date(year, 9, 31, 14, 0, 0, 0); // Month is 0-based; 9 => October
    if (target.getTime() <= now.getTime()) {
      target = new Date(year + 1, 9, 31, 14, 0, 0, 0);
    }
